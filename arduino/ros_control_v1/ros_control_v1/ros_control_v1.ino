...
double setpoint_left = 0; // giá trị setpoint cho motor trái
double setpoint_right = 0; // giá trị setpoint cho motor phải
...
//khai báo hàm moveCallback để xử lý tín hiệu điều khiển nhận được từ move_base_msgs/MoveBaseAction
void moveCallback(const move_base_msgs::MoveBaseAction& msg) {
  // Lấy giá trị tọa độ x và y từ tín hiệu điều khiển
  double x = msg.target_pose.pose.position.x;
  double y = msg.target_pose.pose.position.y;

  // Tính toán giá trị setpoint mới cho motor trái và phải
  setpoint_left = x - y;
  setpoint_right = x + y;

  // Tính toán gia trị vận tốc động cơ 
  vt_DC1 = setpoint_left * speed_pwm;
  vt_DC2 = setpoint_right * speed_pwm;
}

// Khai báo các biến ROS
ros::NodeHandle nh;
// move_base_msgs::MoveBaseAction action;
ros::Subscriber<move_base_msgs::MoveBaseAction> sub("move_base", &moveCallback);
