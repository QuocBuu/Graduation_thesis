...
void moveCallback(const geometry_msgs::Twist& msg) {
  ...
  if (speed_lin != 0) {
    if (speed_ang > 0) {      // phải 
      speed_pwm_right = speed_pwm*(speed_ang-1);
      speed_pwm_left = speed_pwm*speed_ang;
    }
    if (speed_ang < 0) {      // trái
      speed_pwm_right = speed_pwm*(-speed_ang);
      speed_pwm_left = speed_pwm*(1+speed_ang);
    }
    if (speed_ang == 0) {
      speed_pwm_right = speed_pwm;
      speed_pwm_left = speed_pwm;
    }
    ...
    if (speed_ang > 0) {      // phải 
      vt_DC1 = -speed_ang_pwm;
      vt_DC2 = -speed_ang_pwm;
    }
    if (speed_ang < 0) {      // trái
      vt_DC1 = speed_ang_pwm;
      vt_DC2 = speed_ang_pwm;
